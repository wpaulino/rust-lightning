	...
	L::Target: Logger,
{
	...
	fn funding_transaction_generated_intern<FundingOutput: FnMut(&OutboundV1Channel<SP>) -> Result<OutPoint, &'static str>>(
		...
		match peer_state.channel_by_id.entry(chan.context.channel_id()) {
			hash_map::Entry::Occupied(_) => panic!("Generated duplicate funding txid?"),
			hash_map::Entry::Vacant(e) => e.insert(Channel::from(chan)),
		};
		Ok(())
	}
	...
	L::Target: Logger,
{
	fn read<Reader: io::Read>(reader: &mut Reader, mut args: ChannelManagerReadArgs<'a, M, T, ES, NS, SP, F, R, MR, L>) -> Result<Self, DecodeError> {
...

		{
			...
			// 0.0.102+
			for (channel_id, monitor) in args.channel_monitors.iter() {
				let mut channel_force_closed = false;
				let counterparty_node_id = monitor.get_counterparty_node_id();
				if let Some(peer_state_mtx) = per_peer_state.get(&counterparty_node_id) {
					let mut peer_state_lock = peer_state_mtx.lock().unwrap();
					let peer_state = &mut *peer_state_lock;
					channel_force_closed = !peer_state.channel_by_id.contains_key(channel_id);
				}

				if channel_force_closed {
						...
						if let HTLCSource::PreviousHopData(prev_hop) = &htlc_source {
							if let Some(payment_preimage) = preimage_opt {
								...
								Some((htlc_source, payment_preimage, htlc.amount_msat,
									channel_force_closed, Some(monitor.get_counterparty_node_id()),
