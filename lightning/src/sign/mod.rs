...
pub fn compute_funding_key_tweak(base_funding_secret_key: &SecretKey, splice_parent_funding_txid: &Txid) -> Scalar {
...
impl InMemorySigner {
	...
	/// Holder secret key in the 2-of-2 multisig script of a channel. This key also backs the
 	/// holder's anchor output in a commitment transaction, if one is present.
 	pub fn funding_key(&self, parent_funding_txid: Option<Txid>) -> SecretKey {
		let tweak = parent_funding_txid.map(|txid| {
			compute_funding_key_tweak(&self.funding_key.with_tweak(None), &txid)
		});
...
impl ChannelSigner for InMemorySigner {
	...
	fn pubkeys(
		...
		if parent_funding_txid.is_some() {
			pubkeys.funding_pubkey =
				PublicKey::from_secret_key(secp_ctx, &self.funding_key(parent_funding_txid));
		}
		pubkeys
	}
...
impl EcdsaChannelSigner for InMemorySigner {
	fn sign_counterparty_commitment(
		...
		let funding_key = self.funding_key(channel_parameters.parent_funding_txid);
	...
	fn sign_channel_announcement_with_funding_key(
		&self, channel_parameters: &ChannelTransactionParameters, msg: &UnsignedChannelAnnouncement,
		secp_ctx: &Secp256k1<secp256k1::All>,
	) -> Result<Signature, ()> {
